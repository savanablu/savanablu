...
export async function POST(req: Request) {
  ...
  try {
      ...
      line_items: [
        {
          ...
          price_data: {
            ...
            product_data: {
              ...
              description: `Deposit for ${adultsNum} adults, ${childrenNum} children on ${date}`,
            },
          },
        },
      ],
      ...
      (async () => {
        try {
          await sendEmail({
            to: customerEmail,
            subject:
              "Your tour with Savana Blu – deposit payment link",
            html: bookingDepositCustomerEmail(summary),
          });

          await sendEmail({
            to: "hello@savanablu.com",
            subject: `New tour booking started – ${tour.title}`,
            html: bookingDepositOperatorEmail(summary),
          });
        } catch (err) {
          console.error("Tour booking email error:", err);
        }
      })();
    }
    ...
    console.error("Tour checkout error:", err);
    return NextResponse.json(
