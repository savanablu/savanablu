...
type PackageBookingWidgetProps = {
  packageSlug: string;
  packageTitle: string;
  priceFrom: number; // USD per adult for the itinerary
};
...
export default function PackageBookingWidget({
  ...
  const baseTotalUSD = useMemo(
    () => calculateBookingTotal(priceFrom, adults, children),
    [priceFrom, adults, children]
  );
  ...
  const handleSubmitBooking = async () => {
    ...
    if (!date || !pickupLocation || !pickupTime) {
      alert("Please provide your preferred start date, pick-up location, and pick-up time.");
      return;
    }
  ...
  return (
    <aside className="space-y-3 rounded-2xl border border-sb-mist/80 bg-white/95 p-4 text-xs text-sb-ink shadow-sm">
      ...
      <p className="text-[0.78rem] text-sb-ink/75">
        We&apos;ll confirm your booking by email or WhatsApp. Payment is normally made in Zanzibar, with an optional secure online advance link sent afterwards if you&apos;d like to lock in your dates. Payments are processed in USD – other currencies shown here are indicative.
      </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1">
            ...
            <textarea
              ...
              placeholder={`Hotels you like, budget level, any must-see stops for ${packageTitle}…`}
              className="w-full rounded-md border border-sb-mist/80 bg-sb-shell/60 px-3 py-1.5 text-[0.78rem] outline-none focus:border-sb-lagoon focus:ring-1 focus:ring-sb-lagoon/70"
            />
          </div>
      ...
      {/* Totals */}
      <div className="space-y-1 rounded-xl bg-sb-shell/80 p-3 text-[0.85rem] text-sb-ink/85">
        <p className="font-semibold text-sb-night">Estimated trip value</p>
        <div className="flex items-center justify-between">
          <span>Full itinerary total</span>
          <span className="font-semibold">
            {formatPrice(finalTotalUSD, currency, fx)}
          </span>
        </div>
        <p className="mt-2 text-[0.68rem] text-sb-ink/70">
          We&apos;ll confirm your booking by email or WhatsApp. Payment is normally made in Zanzibar, with an optional secure online advance link sent afterwards if you&apos;d like to lock in your dates.
        </p>
      </div>
      ...
      <div className="mt-3 flex items-start gap-2">
        ...
        >
          Please tick this box to confirm you are a real person making this booking.
        </label>
      </div>
      ...
      <p className="text-[0.65rem] text-sb-ink/60">
        You&apos;ll receive a confirmation email from Savana Blu with all the details once your booking is submitted.
      </p>
    </aside>
  );
}
